module SPI_main(
    ...
    // SHIFT REGISTER
    // We're going to use rspns as the shift register output where we first load
        ...
        case (state)
            ...
            CHILLIN: begin // While chilling, we assert load_SCLK because we want to maintain our SCLK line at high
                ...
                if (wrt) begin // If we get a write instruction, we assert init, which:
                    init = 1'b1; // loads in the cmd into our send/receive register, asserts chip select, 
