...
//        a) Right opening present -> dsrd_hdng -= 0x400 (turn right)
...
module MazeRunner_tb_right_affinity();
  ...
  // Run one right-affinity turn scenario:
  ...
  task automatic ChkTurnCase(input logic lft, input logic rght, input logic cmplt,
    ...
    begin
      ...
      // Apply scenario inputs.  In right-affinity mode (cmd0=0) the MOVE->
      // SOL_CHECK transition condition is (rght_opn | mv_cmplt), so any of
      // the three scenarios below will fire on the next clk edge.
  ...
  initial begin
    ...
    ///////////////////////////////////////////////////////////////////////////
    // STEP 2: send maze-solve RIGHT-affinity command (opcode 3'b011, cmd[0]=0
    // => 0x6000).  Immediately neutralize the maze_solve open/cmplt inputs
    ...
    ///////////////////////////////////////////////////////////////////////////
    // STEP 3: cmd[0] selects right affinity.
    //
    // In maze_solve.sv:
    //   assign stp_lft  = cmd0;
    //   assign stp_rght = ~cmd0;
    //
    // For cmd[0]=0 (right affinity) we therefore expect:
    //   * iDUT.iSLV.cmd0     == 0
    //   * iDUT.iSLV.stp_lft  == 0
    //   * iDUT.iSLV.stp_rght == 1
    //
    ...
    // (a) maze_solve sees cmd0=0 (latched cmd[0] coming through cmd_proc).
    ...
    // (b) maze_solve drives stp_lft=0, stp_rght=1 (right-affinity = ignore
    //     right opening when stopping... matches stp_rght = ~cmd0).
    ...
    //     side (because cmd_md is low) and therefore also report 0,1.
    ...
    //
    // Recall the SOL_CHECK arm for cmd0=0:
    //     if (rght_opn)       dsrd_hdng_tmp = dsrd_hdng - 12'h400;  // right
    //     else if (lft_opn)   dsrd_hdng_tmp = dsrd_hdng + 12'h400;  // left
    //     else                dsrd_hdng_tmp = dsrd_hdng + 12'h800;  // 180
    //
    // Case (a): a right opening exists -- the FSM must always prefer it.
    //           rght=1 alone is enough to trigger MOVE -> SOL_CHECK because
    //           in right affinity the MOVE exit cond is (rght_opn | mv_cmplt).
    ///////////////////////////////////////////////////////////////////////////
    $display("[%0t] STEP 5a: right opening -> -0x400 (turn right)", $time);
    ChkTurnCase(.lft(1'b0), .rght(1'b1), .cmplt(1'b0),
                .delta(12'hC00),  // -0x400 mod 0x1000
                .label("STEP 5a RIGHT-TURN"));

    ///////////////////////////////////////////////////////////////////////////
    // Case (b): no right opening, left opening present, move-complete fires
    //           (we hit a wall in front so navigate signaled mv_cmplt).
    //           The right-affinity priority then falls through to the left
    ...
    // sol_cmplt.  With (lft_opn, rght_opn, mv_cmplt) = (0,0,1) and cmd0=0
    // the MOVE exit cond (rght_opn | mv_cmplt) is true on the next clk.
