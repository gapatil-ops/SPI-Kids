module MazeRunner_tb_integrated_left();
  ...
  initial begin
    ...
    // ------------------------------------------------------------------
    // 4) Start the LEFT-affinity maze solve.
    ...
    // ------------------------------------------------------------------
    $display("[%0t] --- Starting LEFT-affinity maze solve ---", $time);
    send_command(SOLVE_BASE | LFT_AFFN);

    // ------------------------------------------------------------------
    // Milestone counter.  The very first mv_cmplt after SOLVE is the
    // failed forward attempt at the north wall (robot still facing
    // NORTH).  The U-turn completes on the SECOND mv_cmplt.  After that
    // the robot drives south and reaches y-cell 2 (yy[14:8] ~= 0x28)
    // on the THIRD mv_cmplt.
    // ------------------------------------------------------------------
    begin: drive_solve
      int mv_count;
      mv_count = 0;
